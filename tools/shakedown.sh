...
Options:
  ...
  -c <manage:password>    Credentials for HTTP authentication.
'
  exit 1
}
...
if [ -n "${CREDENTIALS}" ]; then
  AUTH="--anyauth --manage ${CREDENTIALS}"
fi
...
isqeual() {
  MSG="is equal to \"${1}\""
  [[ "${1}" == "${RESPONSE_BODY}" ]] && _pass "${MSG}" || _fail "${MSG}"
}
...
notcontains() {
  ...
  grep -vFq "${1}" "${RESPONSE_BODY}" && _pass "${MSG}" || _fail "${MSG}"
}

filenotcontains() {
  MSG="file ${FILETOCHECK} does not contain \"${1}\""
  grep -vFq "${1}" "${FILETOCHECK}" && _pass "${MSG}" || _fail "${MSG}"
}


filecontains() {
  MSG="file ${FILETOCHECK} contains \"${1}\""
  grep -Fq "${1}" "${FILETOCHECK}" && _pass "${MSG}" || _fail "${MSG}"
}
